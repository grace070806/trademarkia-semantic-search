...
class SemanticCache:
    ...
    CORRECTION — cluster lookup explanation:
    Instead of scanning the entire cache (O(N)), lookup is restricted to
    entries belonging to the query's dominant cluster and any secondary
    clusters with membership > secondary_threshold. This reduces lookup
    complexity from O(N) to roughly O(N/K), where K is the number of
    clusters. At K=15 with 1,000 cached entries, the average lookup
    touches ~67 entries instead of 1,000 — a 15× speedup that scales
    linearly with K.

    Threshold θ — the critical tunable
    ────────────────────────────────────
    θ controls cache sensitivity:

        θ = 0.95 → near-identical queries only (very strict)
        θ = 0.85 → paraphrases and synonym variants are captured
        θ = 0.70 → broader topical matches (risk of false positives)
        θ < 0.60 → effectively cluster-level cache; too aggressive

    Experiments show θ≈0.85 provides the best balance between precision
    (avoiding false cache hits) and cache hit rate (avoiding redundant
    computation). See analysis/threshold_experiment.png for evidence.

    Cosine similarity computation
    ──────────────────────────────
    CORRECTION: Since all embeddings are L2-normalised before storage,
    cosine similarity between two vectors reduces to their dot product:

        cos_sim(a, b) = (a · b) / (||a|| · ||b||) = a · b  (when ||a||=||b||=1)

    This makes each comparison O(D) with no division — important for
    efficient lookup across potentially large cache buckets.
